...
import {
  ...
  Heart, Mountain, Pill, Brain, PowerOff, FileText, AlertTriangle, Factory, Landmark, Crosshair
} from "lucide-react";
      ...
      <header className="border-b border-border bg-card/60 backdrop-blur-xl sticky top-0 z-40">
        <div className="container mx-auto max-w-6xl flex items-center justify-between h-14 px-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="ghost" size="sm" onClick={() => navigate("/dashboard")}><ArrowLeft className="h-4 w-4 mr-1" /> Dashboard</Button>
          </div>
        </div>
      </header>
      ...
      <main className="container mx-auto max-w-6xl px-4 py-6 space-y-6">
        ...
        {silos.length > 1 && (
          <div className="flex gap-2 flex-wrap">
            {silos.map(s => {
              ...
              return (
                <Button key={s.id} variant={activeSilo === s.id ? "default" : "outline"} size="sm" onClick={() => setActiveSilo(s.id)} className="text-xs">
          ...
          <Card>
            <CardContent className="pt-4 pb-4 space-y-2">
              <div className="flex items-center justify-between">
                <p className="text-sm font-semibold text-foreground">Compliance Integrity</p>
                <span className="text-sm font-bold text-primary">{avgScore}%</span>
              </div>
              <Progress value={avgScore} className="h-2" />
              <p className="text-xs text-muted-foreground">{currentSilo.description}</p>
            </CardContent>
          </Card>
        )}
        ...
        <Tabs defaultValue="records">
          <TabsList className="bg-muted">
            <TabsTrigger value="records"><FileText className="h-3.5 w-3.5 mr-1.5" /> Records</TabsTrigger>
                  ...
                  <TableBody>
                    {currentData.map(d => (
                      <TableRow key={d.id}>
                        ...
                        <TableCell>
                          <Badge className={
                            d.status === "active" ? "bg-primary/20 text-primary text-[10px]"
                  ...
                  <TableBody>
                    {revenue.map(r => (
                      <TableRow key={r.id}>
                        ...
                        <TableCell>
                          <Badge className={r.status === "paid" ? "bg-primary/20 text-primary text-[10px]" : "bg-muted text-muted-foreground text-[10px]"}>
                            {r.status}
                          </Badge>
                        </TableCell>
                      </TableRow>
